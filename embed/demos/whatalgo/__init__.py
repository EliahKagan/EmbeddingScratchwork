...
def define_names(names, *, data_dir=None):
    ...
    old_definitions = _load_definitions(path)
    new_definitions = {}
    results = []  # We populate this with all definitions, but not the names.
    ...
    if not new_definitions:
        _logger.info('%s: no new definitions to save', define_names.__name__)
        return results

    _save_definitions(path, old_definitions | new_definitions)

    if len(new_definitions) == 1:
        _logger.info('%s: saved %d new definition',
                     define_names.__name__, len(new_definitions))
    else:
        _logger.info('%s: saved %d new definitions',
                     define_names.__name__, len(new_definitions))
